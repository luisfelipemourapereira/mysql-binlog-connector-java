...
public class BinaryLogClient implements BinaryLogClientMXBean {
  ...
  /**
   * CONSTRUCTORS Alias for BinaryLogClient("localhost", 3306, &lt;no schema&gt; = null, username,
   * password).
   *
  ...
  /*
   * Constructor adding protocol
   */
  public BinaryLogClient(
      String username, String password, String hostname, int port, String schema) {
    this.hostname = hostname;
    this.username = username;
    this.password = password;
    this.schema = schema;
  ...
  /**
   * @param blocking blocking mode. If set to false - BinaryLogClient will disconnect after the last
   *     event.
   */
   ...
   *           position). This is what happens by default when you don't specify binary log filename
   *           explicitly.
  ...
  /**
   * @param binlogPosition binary log position. Any value less than 4 gets automatically adjusted to
   *     4 on connect.
   ...
   *     <p>NOTE #1: Any value but null will switch BinaryLogClient into a GTID mode (this will also
   *     set binlogFilename to "" (provided it's null) forcing MySQL to send events starting from
   *     the oldest known binlog (keep in mind that connection will fail if gtid_purged is anything
   *     but empty (unless {@link #setGtidSetFallbackToPurged(boolean)} is set to true))).
   *     <p>NOTE #2: GTID set is automatically updated with each incoming GTID event (provided GTID
   *     mode is on).
   * @see #getGtidSet()
  ...
  /**
   * @param gtidSetFallbackToPurged true if gtid_purged should be used as a fallback when gtidSet is
   *     set to "" and MySQL server has purged some of the binary logs, false otherwise (default).
   */
   ...
   *     from a given {@link #getBinlogFilename()} and {@link #getBinlogPosition()} instead of "the
   *     oldest known binlog" when {@link #getGtidSet()} is set, false otherwise (default).
   */
      ...
      try {
        try {
          ...
          if (connectTimeout > 0 && !isKeepAliveThreadRunning()) {
            cancelDisconnect =
                scheduleDisconnectIn(connectTimeout - (System.currentTimeMillis() - start));
          }
  ...
  private void checkError(byte[] packet) throws IOException {
    if (packet[0] == (byte) 0xFF /* error */) {
      ...
      throw new ServerException(
          errorPacket.getErrorMessage(), errorPacket.getErrorCode(), errorPacket.getSqlState());
    }
  }
  ...
  private void ensureEventDataDeserializer(
    ...
    if (eventDataDeserializer.getClass() != eventDataDeserializerClass
      ...
      eventDeserializer.setEventDataDeserializer(
          eventType,
          new EventDataWrapper.Deserializer(internalEventDataDeserializer, eventDataDeserializer));
    }
  }
          ...
          public void run() {
            try {
              ...
              exceptionReference.set(
                  new IOException(
                      e)); // method is asynchronous, catch all exceptions so that they are not lost
              countDownLatch.countDown(); // making sure we don't end up waiting whole "timeout"
            }
          }
        };
              ...
              eventDeserializer.nextEvent(
                  packetLength == MAX_PACKET_LENGTH
                      ? new ByteArrayInputStream(
                          readPacketSplitInChunks(inputStream, packetLength - 1))
                      : inputStream);
          if (event == null) {
            throw new EOFException();
          }
  ...
  /**
   * Register event listener. Note that multiple event listeners will be called in order they where
   * registered.
   *
  ...
  public interface LifecycleListener {
    ...
    /**
     * Called in case of failed event deserialization. Note this type of error does NOT cause client
     * to disconnect. If you wish to stop receiving events you'll need to fire client.disconnect()
     * manually.
     *
